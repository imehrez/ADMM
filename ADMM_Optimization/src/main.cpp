...
/*
 * log the given string to a local file
 */
...
viennacl::compressed_matrix<float> hadamard_matrix_vector (Eigen::MatrixXf & mat, viennacl::vector<float> & vec_gpu){ // fixme: make it pass by reference
	int n_cols = mat.cols();
	int n_rows = mat.rows();
	Eigen::MatrixXf result_mat(n_rows, n_cols);
	viennacl::vector<float> coli_gpu(n_rows), tmp_vec_gpu(n_rows);
	Eigen::VectorXf tmp_col_cpu(n_rows);
	for(int i = 0; i<n_cols; i++)
	{
		tmp_col_cpu = mat.col(i);
		copy(tmp_col_cpu, coli_gpu);
		tmp_vec_gpu = element_prod(coli_gpu, vec_gpu);
		copy(tmp_vec_gpu, tmp_col_cpu);
		result_mat.col(i) = tmp_col_cpu;
//		std::cout << "mat after: " << result_mat << std::endl;
	}
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> result_sparse = result_mat.sparseView();
	viennacl::compressed_matrix<float> result_gpu(n_rows, n_cols);
	copy(result_sparse, result_gpu);
	return result_gpu;
}

/*
 * function for fast hadamard product between a two vectors
 */
viennacl::compressed_matrix<float> hadamard_matrix_vector (cl::Kernel & HadamardMatrixVector,cl::CommandQueue & queue,cl::Context & context,
															Eigen::MatrixXf & mat, viennacl::vector<float> & vec_gpu){
	const int n_cols = mat.cols();
	const int n_rows = mat.rows();


	Eigen::VectorXf matVec(Eigen::Map<Eigen::VectorXf>(mat.data(),n_cols*n_rows)); //
	...
	std::size_t countX = wgSizeX * n_cols/16; // Overall number of work items in X direction = Number of elements in X direction
	std::size_t countY = wgSizeY * n_rows/16;
	std::size_t count = countX * countY; // Overall number of elements
	std::size_t size = count * sizeof (float); // Size of data in bytes
	std::size_t size_vec = n_rows*sizeof(float);

	// Allocate space for output data from CPU and GPU on the host
	std::vector<float> h_outputGpu (count);

	// Allocate space for output data on the device
	cl::Buffer d_mat_input(context, CL_MEM_READ_WRITE, size);
	cl::Buffer d_vec_input(context, CL_MEM_READ_WRITE, size_vec);
	cl::Buffer d_mat_output(context, CL_MEM_READ_WRITE, size);

	// Initialize memory to 0xff (useful for debugging because otherwise GPU memory will contain information from last execution)
	memset(h_outputGpu.data(), 255, size);
	queue.enqueueWriteBuffer(d_mat_input, true, 0, size, matVec.data());
	queue.enqueueWriteBuffer(d_vec_input, true, 0, size_vec, vec.data());
	queue.enqueueWriteBuffer(d_mat_output, true, 0, size, h_outputGpu.data());

	// Launch kernel on the device
	HadamardMatrixVector.setArg< cl::Buffer >(0, d_mat_input);
	HadamardMatrixVector.setArg< cl::Buffer >(1, d_vec_input);
	HadamardMatrixVector.setArg< cl::Buffer >(2, d_mat_output);
	cl::Event eventGPU;
	queue.enqueueNDRangeKernel(HadamardMatrixVector,cl::NullRange,cl::NDRange(countX, countY),
			cl::NDRange(wgSizeX, wgSizeY), NULL, &eventGPU);
	queue.finish();
	// Copy output data back to host
	cl::Event eventRB;
	queue.enqueueReadBuffer(d_mat_output, true, 0, size, h_outputGpu.data(),NULL, &eventRB);
	Eigen::MatrixXf result_kernel= Eigen::Map <Eigen::MatrixXf> (h_outputGpu.data(),n_rows,n_cols);
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> result_sparse = result_kernel.sparseView();
	viennacl::compressed_matrix<float> result_gpu(n_rows, n_cols);
	copy(result_sparse, result_gpu);
	return result_gpu;
...
/*
 * function for fast hadamard product between a matrix and a vector
 */
void hadamard_matrix_vector_timed (cl::Kernel & HadamardMatrixVector,cl::CommandQueue & queue,cl::Context & context,
		Eigen::MatrixXf & mat, viennacl::vector<float> & vec_gpu,
		viennacl::compressed_matrix<float> & result_gpu){
	const int n_cols = mat.cols();
	const int n_rows = mat.rows();
	...
	queue.enqueueWriteBuffer(d_mat_input, true, 0, size, mat.data());
	queue.enqueueWriteBuffer(d_vec_input, true, 0, size_vec, vec.data());
	queue.enqueueWriteBuffer(d_mat_output, true, 0, size, h_outputGpu.data());
	...
	cl::Event eventRB;
	//int start5=clock();
	Eigen::MatrixXf result_kernel(n_rows,n_cols);
	queue.enqueueReadBuffer(d_mat_output, true, 0, size, result_kernel.data(),NULL, &eventRB);
 ...
 */
Eigen::VectorXf PD_Ui_faster(Eigen::VectorXf & z_i, Eigen::SparseMatrix<float, Eigen::RowMajor, int> & T,
		Eigen::VectorXf & x, Eigen::VectorXf & p, float rho, int I0, int n)
{
 ...
 */
void Newton_faster(Eigen::VectorXf & z_i, viennacl::vector<float>& z_i_gpu,
		...
		Eigen::MatrixXf & BT_i, viennacl::compressed_matrix<float> & BT_i_gpu,
				...
				Eigen::VectorXf & X_i, Eigen::VectorXf & P_i ,float rho ,float stepSize, Eigen::VectorXf & rhoEig,int I0,
				int n, int m, int useHadamardKernel, cl::Kernel & HadamardMatrixVector,cl::CommandQueue & queue,cl::Context & context){

	// ----------------------------------------------------------------------------------------------------------- //
	// --------------------------------- Hessian matrix of L2-Norm : slide 22+23 --------------------------------- //
	// ----------------------------------------------------------------------------------------------------------- //
	...
	// diag[-exp(-z_i)]
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> diag_exp_neg(n,n);
	diag_exp_neg = -1*neg_exp_z_i.asDiagonal();
	Eigen::MatrixXf diag_exp_neg_matXf = diag_exp_neg;

	// diag[exp(-z_i)]
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> diag_exp(n,n);
	diag_exp = neg_exp_z_i.asDiagonal();
	Eigen::MatrixXf diag_exp_matXf = diag_exp;
	viennacl::compressed_matrix<float> diag_exp_gpu(n,n);
	copy(diag_exp, diag_exp_gpu);

	// get the partial derivatives
	viennacl::vector<float> pdgi = PD_Gi_faster(z_i_gpu, AT_i_gpu, BT_i_gpu, neg_exp_z_i_gpu, numerator_gpu,Mi_gpu, I0, n, m);
	viennacl::vector<float> pdhi = PD_Hi_faster(z_i_gpu, BT_i_gpu, neg_exp_z_i_gpu, Mi_gpu, I0);

	viennacl::vector<float> sum_pd = pdgi +pdhi;

	// 2nd derivative of dG_i/dz_i
	// ----------------------------------------------------------------------------------------------------------- //
	viennacl::compressed_matrix<float> Hessian_G_1(n,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, diag_exp_neg_matXf, sum_pd, Hessian_G_1);

	viennacl::compressed_matrix<float> combo_exp_part1_tmp_gpu(m,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, A_i,Mi_gpu, combo_exp_part1_tmp_gpu);
	viennacl::compressed_matrix<float> combo_exp_part1 = prod(AT_i_gpu, combo_exp_part1_tmp_gpu);

	viennacl::vector<float> M_denom_sq_gpu = hadamard_vector_vector(denominator_gpu, denominator_gpu);
	viennacl::vector<float> M_denom_cub_gpu = hadamard_vector_vector(M_denom_sq_gpu, denominator_gpu);
	viennacl::vector<float> numerator_sq_gpu = hadamard_vector_vector(numerator_gpu,numerator_gpu);

	viennacl::vector<float> num_denom_sq_gpu(m), num_sq_denom_cub_gpu(m);
	num_denom_sq_gpu = element_div(numerator_gpu,M_denom_sq_gpu);
	num_sq_denom_cub_gpu = element_div(numerator_sq_gpu,M_denom_cub_gpu);

	viennacl::compressed_matrix<float> combo_exp_part3_tmp_gpu(m,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, A_i,num_denom_sq_gpu, combo_exp_part3_tmp_gpu);
	viennacl::compressed_matrix<float> combo_exp_part3 = prod(BT_i_gpu, combo_exp_part3_tmp_gpu);

	viennacl::compressed_matrix<float> combo_exp_part4_tmp_gpu(m,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, B_i,num_sq_denom_cub_gpu,combo_exp_part4_tmp_gpu);
	viennacl::compressed_matrix<float> combo_exp_part4 = prod(BT_i_gpu,combo_exp_part4_tmp_gpu);

	viennacl::compressed_matrix<float> combo_exp_part2_tmp_gpu(m,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, B_i,num_denom_sq_gpu,combo_exp_part2_tmp_gpu);
	viennacl::compressed_matrix<float> combo_exp_part2 = prod(AT_i_gpu,combo_exp_part2_tmp_gpu);

	int I0_sq = I0*I0;
	int size = n;
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> combo_exp_part1_tmp(size,size), combo_exp_part2_tmp(size,size), combo_exp_part3_tmp(size,size), combo_exp_part4_tmp(size,size);
	copy(combo_exp_part1,combo_exp_part1_tmp); // convert to Eigen to add
	copy(combo_exp_part2,combo_exp_part2_tmp);
	copy(combo_exp_part3,combo_exp_part3_tmp);
	copy(combo_exp_part4,combo_exp_part4_tmp);
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> Hessian_G_2_tmp_cpu = combo_exp_part1_tmp + combo_exp_part2_tmp + combo_exp_part3_tmp + combo_exp_part4_tmp;
	Hessian_G_2_tmp_cpu = I0_sq*Hessian_G_2_tmp_cpu;
	viennacl::compressed_matrix<float> Hessian_G_2_tmp1(size,size), Hessian_G_2_tmp2(size,size), Hessian_G_2(size,size);
	copy(Hessian_G_2_tmp_cpu, Hessian_G_2_tmp1);
	Hessian_G_2_tmp2 = prod(Hessian_G_2_tmp1, diag_exp_gpu);
	Hessian_G_2 = prod(diag_exp_gpu, Hessian_G_2_tmp2);
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> Hessian_G_cpu(size,size), Hessian_G_1_tmp_cpu(size,size);
	copy(Hessian_G_1, Hessian_G_1_tmp_cpu);
	copy(Hessian_G_2, Hessian_G_2_tmp_cpu);

	Hessian_G_cpu = Hessian_G_1_tmp_cpu + Hessian_G_2_tmp_cpu;


	// 2nd derivative of dH_i/dz_i
	// ----------------------------------------------------------------------------------------------------------- //
	viennacl::vector<float> tempH1 = I0*prod(BT_i_gpu,Mi_gpu);
	viennacl::compressed_matrix<float> Hessian_H_1(n,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, diag_exp_matXf, tempH1,Hessian_H_1);

	Eigen::SparseMatrix<float, Eigen::RowMajor, int> Hessian_H_1_cpu(n,n), Hessian_H_2_cpu(n,n);
	copy(Hessian_H_1, Hessian_H_1_cpu);

	viennacl::compressed_matrix<float> tempH2_(m,n);
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, B_i,M_denom_sq_gpu,tempH2_);

	viennacl::compressed_matrix<float> tempH2 = prod(BT_i_gpu, tempH2_);
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> tempH2_cpu_sparse(n,n);
	copy(tempH2,tempH2_cpu_sparse);
	tempH2_cpu_sparse = I0_sq*tempH2_cpu_sparse;
	Eigen::MatrixXf tempH2_cpu = tempH2_cpu_sparse;
	hadamard_matrix_vector_timed(HadamardMatrixVector, queue, context, tempH2_cpu,neg_exp_z_i_gpu,tempH2);

	viennacl::compressed_matrix<float> Hessian_H_2_ = prod(tempH2,diag_exp_gpu);
	viennacl::compressed_matrix<float> Hessian_H_2 = prod(diag_exp_gpu,Hessian_H_2_);
	copy(Hessian_H_2, Hessian_H_2_cpu);
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> Hessian_H_cpu = 0.5*Hessian_H_1_cpu + Hessian_H_2_cpu;


	// 2nd derivative of dU_i/dz_i
	// ----------------------------------------------------------------------------------------------------------- //
	Eigen::SparseMatrix<float, Eigen::RowMajor, int> Hessian_U_cpu(n,n);
	Hessian_U_cpu = rhoEig.asDiagonal();

	Eigen::SparseMatrix<float, Eigen::RowMajor, int> hessian = Hessian_G_cpu + Hessian_H_cpu + Hessian_U_cpu;
	...
	Eigen::VectorXf pdui = PD_Ui_faster(z_i, T_i, X_i, P_i, rho, I0, n);

	sum_pd_cpu += pdui;
	sum_pd_cpu = sum_pd_cpu*-1*stepSize; //tune step size

	//Solve using conjugate gradient
	Eigen::VectorXf d_k = viennacl::linalg::solve(hessian, sum_pd_cpu, viennacl::linalg::cg_tag() ); //viennacl::linalg::cg_tag()

	if(! std::isnan(d_k[0]))
		z_i = z_i + d_k; // update z_k by reference
}
...
/*
 * used in the calculation of the energy function
 */

float calculate_g_i(Eigen::VectorXf & z_i, viennacl::vector<float>& z_i_gpu,
							Eigen::MatrixXf & A_i, viennacl::compressed_matrix<float> & A_i_gpu,
							Eigen::MatrixXf & AT_i, viennacl::compressed_matrix<float> & AT_i_gpu,
							Eigen::MatrixXf & B_i, viennacl::compressed_matrix<float> & B_i_gpu,
							Eigen::MatrixXf & BT_i, viennacl::compressed_matrix<float> & BT_i_gpu,
									...
									int n, int m)
{
		...
		viennacl::vector<float> sigma_sq_gpu(m);
		copy(sigma_sq, sigma_sq_gpu);
		Eigen::VectorXf Aik_cpu(n); //kth row
		viennacl::vector<float> Aik_gpu(n);
		Eigen::VectorXf Bik_cpu(n);//kth row
		viennacl::vector<float> Bik_gpu(n);
		viennacl::vector<float> neg_exp_z_i_gpu(n);
...
int main(int argc, char** argv) {
	...
	int useHadamardKernel = 1;   // Hadamard matrix vector product: 1->use custom kernel, 0->use Eigen+viennacl function (slower)
	...
	std::cin >> sigma;
	std::cout<<"Enter float rho in the range of 0.1 to 10"<<std::endl;
	std::cin >> rho;
	std::cout<<"Enter float step size in the range of 1e-10 to 1e-3"<<std::endl;
	std::cin >> stepSize;
	...
	log("Step size:"+ boost::lexical_cast <std::string >(stepSize)+"\n");
	...
	cl::Kernel HadamardMatrixVector (program, "HadamardMatrixVector");
	...
	for (int i = 0; i < M; i++)
	{
		Src[i] = cv::imread(
				"../Images/PCB/LR" + boost::lexical_cast<std::string>(i + 1)
						+ ".tif", CV_LOAD_IMAGE_ANYDEPTH   ); //CV_LOAD_IMAGE_GRAYSCALE fixme
	...
	gnd_truth = cv::imread(
					"../Images/PCB/origin.tif", CV_LOAD_IMAGE_ANYDEPTH);//CV_LOAD_IMAGE_GRAYSCALE
...
//	x = -1*element_log((x/I_0));
	...
	// ------------------------------------------------------------------------------------------ //
	// -------------------------------- Newton's method Initializations ------------------------- //
	// ------------------------------------------------------------------------------------------ //
	...
	if(iterate_flag==1)
	{
		...
		for(int iter = 0; iter<n_iterations; iter++)
		{
			...
			for (int i = 0;  i < i_uBound; i++) {
				if(i < M) // Newton Step Update
				{
					//std::cout << "Newton step: " << i << std::endl;
					//int c_newton_start=clock();
					Newton_faster(z_k[i] , z_k_viennacl[i],
							A_i[i], DHF[i],
							AT_i[i], DHFT[i],
							B_i[i],DHF2[i],
							BT_i[i], DHFT2[i],
							Y[i], y_viennacl[i],
							sigmasqEig, T[i], x_k, p_k[i], rho,stepSize, rhoEig, I_0, n, m,
							useHadamardKernel, HadamardMatrixVector, queue, context);
			...
			//std::cout << "p update -> done"  << std::endl;
			...
			for(int i=0; i<i_uBound;i++)
			{
				...
				if(i < M )
				{
					...
					g_i[i] = calculate_g_i(z_k[i] , x_k_vienna, A_i[i], DHF[i],
							AT_i[i], DHFT[i],
							B_i[i],DHF2[i],
							BT_i[i], DHFT2[i],
							Y[i], y_viennacl[i],
							sigmasqEig, T[i], x_k, p_k[i], rho, rhoEig, I_0, n, m);
					//log_cost(" "+ boost::lexical_cast <std::string >(g_i[i])+" ");
			...
			std::string imageName = "output_imgs/output_newton" + boost::lexical_cast<std::string> (iter) + ".tif";
